...
module.exports = function(status, song, onDone) {
  ...
  console.log(
    chalk.gray.dim(path.dirname(song.path).replace(homePath, '~') + path.sep)+
    chalk.white( path.basename(song.path, path.extname(song.path)) ) +
    chalk.gray.dim(' [') + (song.rating || '0') + chalk.gray.dim(']'));
  console.log(status + 'Played: ' + (song.playCount || '0') + 'x, last played ?? days ago');
