...
module.exports = function(songs, currentSong, maxCols, useUtf8) {
  ...
  if (maxCols < 60 && maxLength() > maxCols && useUtf8) {
    playCounts = songs.map(function(song) {
      return ' ' + chalk.white.bold('♫') + song.playCount || 0;
    });
  }

  if (maxCols < 60 && maxLength() > maxCols) {
    ratings = songs.map(function(song) {
      return ' ' + chalk.yellow.bold(starChar) + song.rating || 0;
    });
  }
