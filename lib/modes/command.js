...
module.exports = function(opts) {
  ...
  function list(currentSong) {
    ...
    console.log(rows.map(function(row, index) {
      var song = list[index];
      var i = 0;
      if (row.slice(0, -2).join('').length - 9 - 1 >= process.stdout.columns) {
        // the filename is too long
        var filenameIndex = (row.length >= 4 ? 1 : 0);
        row[filenameIndex] = row[filenameIndex].substr(0, process.stdout.columns - row.slice(0, -2).join('').length - 9 - 3 - 1) + '...';
      }
      ...
      if (useUtf8) {
        ...
        padding = rowLength - 5 - 4;
      } else {
        // 3 = [n], 3 = play count
        padding = rowLength - 3 - 3;
      }
      // remove one character (e.g on windows the terminal doesn't like full length lines)
