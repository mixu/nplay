...
module.exports = function(playlist) {
  ...
  var listeners = [
      new CommandMode({ playlist: playlist }),
      new JumpMode({ playlist: playlist })
  ];
  ...
  process.stdin.on('keypress', function(chunk, key) {
    ...
    if (key && key.name == 'j') {
      inputMode = 'jump';
      console.log('? ');
      listeners[1].keypress(null, null, true);
      return;
    }
    ...
    if (inputMode === 'command') {
      ...
      handled = listeners[1].keypress(chunk, key, false, function(err, song) {
        inputMode = 'command';
        if (song) {
          playlist.jump(song);
          setTimeout(function() { listeners[0].keypress('x', { name: 'x' }); }, 10); // play
